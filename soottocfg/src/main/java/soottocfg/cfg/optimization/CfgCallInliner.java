...
public class CfgCallInliner {
    ...
    private void inlineCalls(Method method, int maxSize, int maxOccurences) {
        ...
        if (method.getExitBlocks().size() > 1) {
            System.err.print("Method " + method.getMethodName());
            System.err.println("had more than one exit block: " + method.getExitBlocks().size());
            System.err.println("\t" + String.join(", ", method.getExitBlocks()
                                                              .stream().map(CfgBlock::getLabel)
                                                              .collect(Collectors.toList())));
            method.findOrCreateUniqueSink();
            System.err.println("Unified exits " + method.getExitBlocks().size());
        }
    }
