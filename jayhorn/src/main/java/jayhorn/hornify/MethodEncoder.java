...
public class MethodEncoder {
	...
	public MethodEncoder(Prover p, Program program, Method method, ClassType cType) {
		...
		for (Variable v : methodPreVariables)
			methodPreExprs
					.add(p.mkHornVariable(v.getName() + "_" + HornHelper.hh().newVarNum(), getProverType(v.getType())));
	}
	...
	private void createVarMap(List<Variable> cfgVars, List<ProverExpr> proverVars, Map<Variable, ProverExpr> varMap) {
		// for (Variable v : cfgVars) {
		// ProverExpr e = varMap.get(v);
		// if (e == null) {
		// e = p.mkHornVariable(v.getName() + "_" + HornHelper.hh().newVarNum(),
		// getProverType(v.getType()));
		// varMap.put(v, e);
		// }
		// proverVars.add(e);
		// }
		HornHelper.hh().createVarMap(p, cfgVars, proverVars, varMap);
	}
		...
		if (s instanceof AssertStatement) {
			...
			for (ClassVariable sig : possibleTypes) {
				...
				while (cnt < totalFields + 1) {
					...
					final ProverExpr lhsExpr = p.mkHornVariable(
							"pullRes_stub" + cnt + "_" + HornHelper.hh().newVarNum(),
			...
			// final ProverExpr invAtom =
			// p.mkOr(invariantDisjunction.toArray(new
			// ProverExpr[invariantDisjunction.size()]));
